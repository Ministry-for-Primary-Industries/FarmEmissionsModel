...
The FEM methodology describes a process for estimating methane, nitrous oxide and carbon dioxide emissions from both livestock (cattle, deer and sheep) and fertiliser. It has been tailored to specific characteristics found in NZ agriculture.
...
This code base is built with and intended to be run on R v4.5.2. The R environment is managed with [renv](https://rstudio.github.io/renv/).
...
- Run the following in the R console to install dependencies:
    ...
    renv::restore()
    ```
- Note an appropriate version of Rtools may be required to compile libraries specified in the renv lockfile. 

### 3. Add Farm Data

Farm data inputs must conform to the [FEM data specification](https://github.com/Ministry-for-Primary-Industries/FarmEmissionsModel/blob/main/FEM_data_specification.md). A supporting data dictionary is also available here providing detailed data definitions.
...
*Note there is currently limited validation performed on input data in FEM. It is the user's responsibility to apply the validation rules contained in the FEM data specification, prior to passing data through FEM.*
...
- output tables to save out via `param_saveout_emission_tables` and `param_saveout_mitign_delta_tables`
...
1.  `utils/FEM_equations.Rmd` is the **original** format, containing the R code with helpful markdown commentary throughout. This is the format we recommend for exploring via IDE.
